describe("Notes", () => {
    ...
    describe("Viewing notes", () => {
        it("should display notes correctly", () => {
            ...
            cy.get(":nth-last-child(1) > .moj-timeline__header > .moj-timeline__title").should(
                "contain",
                "New note title",
            );
    ...
    describe("Adding a note", () => {
        ...
        describe("Successfully adding a note", () => {
            it("has a add a note page with expected fields", () => {
                ...
                cy.get(".govuk-character-count > .govuk-form-group > .govuk-label").should(
                    "contain",
                    "Note (required)",
                );
                cy.get("#f-2-note-info").should("contain", "You have 1000 characters remaining");
        ...
        it("displays an error if the request fails with a 500 error", () => {
            ...
            cy.get("#add-note-form").submit();
            cy.get(".govuk-heading-l").should("contain", "Sorry, there is a problem with the service");
            cy.get(".govuk-body").should("contain", " returned 500");
        });
    });
});
